...
const AIWorkflowLogging = ({
  ...
  function parseArrayFieldsToHTML(obj, options = {}) {
    const {
      fieldsToConvert = [], appendSuffix = "Html"
    } = options
    const result = {
      ...obj
    }
    ...
    fieldsToConvert.forEach((field) => {
      ...
      if (Array.isArray(value) && value.length > 1) {
        result[`${field}${appendSuffix}`] = `<ul>${value.map(item => `<li>${item}</li>`).join("")}</ul>`
  ...
  return ( <
    ...
    p className = "mt-4 mb-6 text-4xl" > {
      item.summary
    } < /p> <
    /div>
    ...
    /div> {
      heroImage && ( <
        GatsbyImage image = {
          heroImage.childImageSharp.gatsbyImageData
        }
        alt = {
    ...
    <
    div className = "grid grid-cols-2 sm:grid-cols-4 items-center gap-9" >
